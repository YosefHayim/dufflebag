...
Place a background region **behind** the figure: the figure's opaque paths cover the part inside its footprint, so only the true background shows. The demo's `extractBlob.ts` is a worked example.
