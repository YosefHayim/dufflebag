#!/usr/bin/env node
/**
 * ctx-loop-ctl — the control plane behind the /autorun, /autostop, /autoexit
 * skills. The skills are thin: they shell out to one of these subcommands.
 * Keeping the logic here (not regenerated per skill) makes arm/pause/exit
 * deterministic and gives all three a single, consistent accounting report.
 *
 ...
 *   stop      /autostop — pause: delete the arm flag (daemon keeps observing), report.
 *   exit      /autoexit — disarm AND tell the daemon to self-terminate, report.
 *
...
function cmdArm(sid: string, args: string[]): void {
  ...
  console.log(
    `🟢 Autorun ARMED — budget ${budget} cycle(s).\n` +
      ...
      "until the budget is spent, you /autostop, or the task is marked done.\n" +
...
function cmdStop(sid: string): void {
  ...
  report(sid, "⏸️  Autorun PAUSED (/autostop) — daemon still observing; /autorun to resume.");
}

function cmdExit(sid: string): void {
  remove(loopFile(sid, "armed"));
  writeText(loopFile(sid, "exit"), ""); // daemon self-terminates on its next poll
  report(sid, "🛑 Autorun EXITED (/autoexit) — daemon shutting down for this session.");
}

function main(): void {
  const cmd = process.argv[2];
  if (!cmd || !["arm", "stop", "exit"].includes(cmd)) {
    console.error("usage: ctx-loop-ctl.js {arm <n>|stop|exit}");
    process.exit(2);
  }
