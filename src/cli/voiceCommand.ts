...
type NarrationMode = BagConfig["speechResponseMode"];
...
export const isTtsNarrationEnabled = (mode: NarrationMode): boolean => mode !== "off";
...
const purgeSpeakRuntimeDirectory = (root: string) =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
...
export const disableVoiceWorker = (scope: CliScope) =>
  Effect.gen(function* () {
    ...
    if (snapshot._tag === "missing" || !snapshot.receipt.features.some((feature) => feature === voiceFeatureId)) {
      ...
      yield* purgeSpeakRuntimeDirectory(location.destination.root);
...
export const writeBagConfigPatch = (scope: CliScope, patch: Partial<BagConfig>) =>
  Effect.gen(function* () {
    ...
    // Structural compare avoids keyof casts on Partial patches.
...
export const writeNarrationMode = (scope: CliScope, mode: NarrationMode) =>
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const watcher = yield* Effect.acquireRelease(PlatformCommand.start(watcherCommand), (process) =>
          process.kill().pipe(Effect.catchAll(() => Effect.void)),
        );
...
const sttMicOffDelayCommand = CliCommand.make(
  ...
  CliCommand.withDescription("Show or set post-release mic open time in ms (catches trailing words; default 200)"),
);
...
const ttsOnCommand = CliCommand.make(
  ...
  (args) =>
    Effect.gen(function* () {
      ...
      const { config, changed } = yield* writeNarrationMode(args.scope, "auto");
      yield* TerminalUI.success(
        changed
...
const ttsOffCommand = CliCommand.make(
  ...
  (args) =>
    Effect.gen(function* () {
      ...
      const { location, changed } = yield* writeNarrationMode(args.scope, "off");
      yield* TerminalUI.success(
        changed
