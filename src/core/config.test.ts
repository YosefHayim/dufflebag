 ...
 * between a BagConfig and the `dufflebag*` string map, and reading effective
...
import { DEFAULTS, fromEnvMap, toEnvMap, validateConfig } from "./config.js";
...
describe("env round-trip", () => {
  ...
  it("fromEnvMap parses values and falls back to defaults for missing keys", () => {
    const cfg = fromEnvMap({ dufflebagContextWarnFraction: "0.12" });
  ...
  it("fromEnvMap ignores garbage and uses the default", () => {
    expect(fromEnvMap({ dufflebagAutorunMaxCycleCount: "abc" }).autorunMaxCycleCount).toBe(DEFAULTS.autorunMaxCycleCount);
  });
});
...
describe("dedup config", () => {
  ...
  it("round-trips mode + skip through the env map", () => {
    expect(toEnvMap({ dedupEnforcement: "deny", dedupSkipDirectories: "templates" })).toEqual({
      ...
      dufflebagDedupSkipDirectories: "templates",
    });
  });
  ...
  it("coerces an unknown env mode back to the safe deny default", () => {
    expect(fromEnvMap({ dufflebagDedupEnforcement: "bogus" }).dedupEnforcement).toBe("deny");
  });
});
