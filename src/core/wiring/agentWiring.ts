 ...
 *    `/dufflebag/` marker, exactly like the Claude hooks.
...
import { isBagCommand } from "../settings/paths.js";
...
export function mergeCursorHook(input: CursorHooksFile, command: string): CursorHooksFile {
  ...
  file.version ??= 1;
  file.hooks ??= {};
  const edits = file.hooks.afterFileEdit;
  if (edits) edits.push({ command });
  else file.hooks.afterFileEdit = [{ command }];
  return file;
}

// --- AGENTS.md managed block ------------------------------------------------

const BLOCK_START = "<!-- dufflebag:dedup-guard start -->";
const BLOCK_END = "<!-- dufflebag:dedup-guard end -->";
...
export const DEDUP_AGENTS_RULE = [
  "## No duplicate code — dufflebag dedup-guard",
  "",
...
/** Locate the managed block as `[start, endExclusive]`, or null when absent. */
function findBlockSpan(text: string): [number, number] | null {
  const start = text.indexOf(BLOCK_START);
  const end = text.indexOf(BLOCK_END);
  if (start !== -1 && end !== -1 && end > start) return [start, end + BLOCK_END.length];
  return null;
}
...
export function upsertManagedBlock(text: string, body: string): string {
  ...
  if (span) return text.slice(0, span[0]) + block(body) + text.slice(span[1]);
...
export function stripManagedBlock(text: string): string {
  const span = findBlockSpan(text);
  if (!span) return text;
  const before = text.slice(0, span[0]).replace(/\n+$/, "");
  const after = text.slice(span[1]).replace(/^\n+/, "");
