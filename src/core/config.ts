 ...
 * rendering a config patch back to the `dufflebag*` string map.
 */

import { DEDUP_MODES, DEFAULTS, ENV_KEYS, ENV_PREFIX, isDedupMode, parseNumber, readConfig } from "../payload/config.js";
import type { BagConfig } from "./catalog/types.js";
...
export function validateConfig(patch: Partial<BagConfig>): Partial<BagConfig> {
  ...
  if (out.contextWarnFraction != null && out.contextBlockFraction != null && out.contextWarnFraction >= out.contextBlockFraction) {
    throw new Error(`contextWarnFraction (${out.contextWarnFraction}) must be below contextBlockFraction (${out.contextBlockFraction})`);
  }
  return out;
}

/** Render a config patch to the `dufflebag*` string→string map written into settings.json `env`. */
