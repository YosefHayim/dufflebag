...
if (!targetArg || !inputArg) {
  fail(
    "usage: tsx src/bin/pixelDiff.ts --target <design.png> --input <build.html|url> " +
...
const url = /^(https?|file):\/\//.test(inputArg) ? inputArg : pathToFileURL(path.resolve(inputArg)).href;
