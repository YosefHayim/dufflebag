...
Key behaviors:
- **Each pick → a rule card.** Chosen variant = the `// ✓` case; rejected variant = the `// ✗` case. Every card follows the fixed five-slot anatomy in [CODE-STYLE-FORMAT.md](_shared/CODE-STYLE-FORMAT.md) (**SSOT root: this package’s format + root `CODE-STYLE.md` + `templates/mdFiles/CODE-STYLE.md` — never an external product repo**): `###` short name (no tags), `[rule:<id>] · verify: \`<command>\`` or `· verify: judgment`, **exactly one assertion sentence**, one fenced `// ✓` + `// ✗` block, `Why:` line. **Never** write `[taste]` / `[lint: …]` / `[CI: …]` in the heading.
