...
2a. The **structured ruleset** `code-style.rules.json` — the machine mirror of `CODE-STYLE.md`, every rule tagged with its enforcement **channel** — and the Biome artifacts it generates (scoped `overrides`, `noRestrictedImports`, `biome-rules/*.grit` plugins). See **[RULESET.md](RULESET.md)** for the channel model, the GritQL facts, and the prove-it gate (zero existing violations + catches a planted one + `verify` stays green). This is the SSOT the `grill-me-code-style-coach` and `grill-me-code-style-review` skills read.
